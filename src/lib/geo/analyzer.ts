...
type EngineCore = Omit<
  GeoAnalysisResult,
  "signals" | "createdAt" | "overallScore" | "aiVisibility"
>;
...
Types JSON-LD détectés: ${signals.jsonLdTypes.join(", ") || "(aucun)"}
...
4. Propose 5 à 8 recommandations actionnables, priorisées (critique/haute/moyenne/basse) avec un impact estimé (1-10) et la catégorie concernée.
5. engines : pour CHACUN des 2 moteurs IA (ChatGPT, Gemini), donne un score GEO (0-100) propre à ce moteur, la position estimée du site dans les citations de ce moteur (estimatedPosition, 1 = cité en premier, null = non cité), un niveau de visibilité (forte/moyenne/faible/absente), un résumé d'une phrase, et competitorsAhead : la liste (0 à 3 noms) des concurrents que CE moteur citerait avant ce site pour une requête typique du secteur. Différencie les moteurs : ChatGPT Search valorise l'autorité et la structure, Gemini s'appuie sur l'écosystème Google et les données structurées.
...
async function analyzeWithClaude(
  ...
  const response = await client.messages.create({
    ...
    messages: [{ role: "user", content: buildPrompt(signals, ctx, profile, mapsListing) }],
  });
  ...
  const parsed = extractJson(textBlock.text) as {
    ...
    engines: (EngineScore & { competitorsAhead?: string[] })[];
  ...
  const ENGINES: EngineScore["engine"][] = ["ChatGPT", "Gemini"];
  const engines: EngineScore[] = ENGINES.map((engine) => {
    ...
    return {
      ...
      competitorsAhead: Array.isArray(found?.competitorsAhead)
        ? found.competitorsAhead.slice(0, 3)
        : [],
    };
  });
  ...
  const engines: EngineScore[] = [
    {
      ...
      engine: "ChatGPT",
      ...deriveEngine(clamp(citability * 0.5 + brandAuthority * 0.3 + technical * 0.2), allCrawlersOk),
    },
    {
      // Gemini : écosystème Google + données structurées
      engine: "Gemini",
      ...deriveEngine(clamp(structuredData * 0.4 + technical * 0.35 + citability * 0.25), allCrawlersOk),
    },
  ];
...
/**
 * Requête envoyée aux moteurs (GPT/Gemini), formulée comme un utilisateur
 * grand public : on demande le TOP 10 du type de commerce dans la niche/zone
 * détectée par Claude EN AMONT. C'est exactement cette requête qu'on « mesure ».
 */
function buildEngineQuery(profile: BusinessProfile): string {
  const niche = profile.niche?.trim() || profile.generalCategory?.trim() || "commerces";
  const loc = profile.isPhysical && profile.location ? ` à ${profile.location}` : "";
  // Format de sortie minimal : parsing fiable + faible coût en tokens.
  return `Donne le classement des 10 meilleurs ${niche}${loc}. Réponds UNIQUEMENT par la liste, un établissement par ligne, au format « 1. Nom de l'établissement », du meilleur au moins bon. Pas d'introduction, pas d'explication, pas de mise en gras, pas de commentaire.`;
}
...
/**
 * Fusionne les positions MESURÉES (réponses réelles GPT/Gemini) dans les scores
 * moteurs estimés par Claude. Le score de préparation GEO reste celui de Claude ;
 * seuls position, concurrents et visibilité passent en « mesuré ».
 */
function mergeMeasuredEngines(
  engines: EngineScore[],
  lives: LiveEngineResult[],
  signals: SiteSignals,
  businessName: string,
): EngineScore[] {
  if (!lives.length) return engines;
  return engines.map((e) => {
    const live = lives.find((l) => l.engine === e.engine);
    if (!live || !live.available || !live.answered) return e;
    const m = measureEngine(live, signals.domain, businessName);
    if (!m.measured) return e;
    return {
      ...e,
      estimatedPosition: m.position,
      competitorsAhead: m.competitorsAhead.length ? m.competitorsAhead : e.competitorsAhead,
...
export async function analyzeSite(
  ...
  if (hasKey) {
    try {
      ...
      profile = await detectProfile(signals, ctx);
    } catch (err) {
  ...
  // ÉTAPE 2 — En parallèle : audit complet (Opus 4.8) + classement réel
  // (OpenAI/Gemini) + estimation backlinks (Claude + recherche web).
  const query = buildEngineQuery(profile);
  geoLog(`Étape 2 — Audit (${MODEL}) + classement moteurs + backlinks en parallèle`);
  geoLog("Requête envoyée aux moteurs (top 10)", query);
  const [core, lives, backlinks] = await Promise.all([
    (async (): Promise<EngineCore> => {
      if (!hasKey) return heuristicAnalysis(signals, ctx, profile);
      try {
        return await analyzeWithClaude(signals, ctx, profile, mapsListing);
      } catch (err) {
    ...
    hasAnyEngineKey()
      ? gatherLiveEngines(query).catch((err) => {
          console.error("Appels moteurs réels échoués :", err);
  ...
  geoLog("Étape 2 — Audit terminé", {
    ...
    qualifications: core.webPresence.qualifications.length,
    classementsLocaux: core.localRankings.length,
  });

  // Fusion des positions mesurées dans les scores moteurs estimés.
  core.engines = mergeMeasuredEngines(core.engines, lives, signals, core.businessName);
  const liveQuery = core.engines.some((e) => e.measured) ? query : null;
  geoLog(
    "Moteurs — positions retenues",
    core.engines.map((e) => ({
      moteur: e.engine,
      mesuré: !!e.measured,
      position: e.estimatedPosition,
      visibilité: e.visibility,
      concurrentsDevant: e.competitorsAhead ?? [],
    })),
  );
  ...
  const aiVisibility = buildAiVisibility(core.engines, core.googleSeo, signals);
  return {
    ...core,
    overallScore,
    aiVisibility,
...
// Part d'usage approximative de chaque IA (pour pondérer les vues estimées).
const ENGINE_MARKET_WEIGHT: Record<AiEngine, number> = {
  ChatGPT: 0.75,
  Gemini: 0.25,
};
...
function buildSampleQueries(
  keywords: string[],
  e: EngineScore,
): AiVisibilityEngine["sampleQueries"] {
  const base = keywords.length
    ? keywords
    : ["votre activité principale", "près de chez moi", "meilleur du secteur"];
  const citedCount =
    e.visibility === "forte" ? 3 : e.visibility === "moyenne" ? 2 : e.visibility === "faible" ? 1 : 0;
  return base.slice(0, 3).map((kw, i) => {
    const cited = i < citedCount && e.estimatedPosition != null;
    return {
      query: kw,
      cited,
      position: cited ? (e.estimatedPosition ?? 1) + i : null,
    };
  });
}

/**
 * Comparatif des vues IA estimées par moteur, modélisé de façon déterministe
 * à partir de la citabilité par moteur (visibilité de citation) et de la part
 * d'usage de chaque IA. Aucune donnée historique inventée.
 */
function buildAiVisibility(
  ...
  // Volume mensuel adressable estimé (proxy de la demande IA sur la thématique).
  const richness = Math.max(0, Math.min(1, signals.wordCount / 1500));
  const baseQueries = Math.round(700 + richness * 2300); // ~700–3000 requêtes/mois
  const keywords = googleSeo.topKeywords.slice(0, 3);

  const list: AiVisibilityEngine[] = engines.map((e) => {
    const citationRate = clamp(e.score); // citabilité estimée par moteur
    const weight = ENGINE_MARKET_WEIGHT[e.engine] ?? 0.2;
    const estimatedMonthlyVisits = Math.round(
      baseQueries * weight * (citationRate / 100),
    );
    const shareOfVoice = clamp(citationRate * 0.9);
    return {
      engine: e.engine,
      citationRate,
      shareOfVoice,
      estimatedMonthlyVisits,
      sampleQueries: buildSampleQueries(keywords, e),
    };
  });

  list.sort((a, b) => b.estimatedMonthlyVisits - a.estimatedMonthlyVisits);
  const total = list.reduce((acc, v) => acc + v.estimatedMonthlyVisits, 0);

  return {
    totalEstimatedMonthlyVisits: total,
    topEngine: list[0]?.engine ?? null,
    engines: list,
    methodologyNote:
      "Estimation modélisée à partir de la citabilité de votre site par moteur (visibilité de citation) et de la part d'usage de chaque IA — ce ne sont pas des visites mesurées. Connectez votre analytics pour des chiffres réels.",
  };
}

...
/**
 * Les questions tapées sous les yeux du visiteur pendant que son analyse tourne.
 *
 * L'écran d'attente montre un clavier et une barre de recherche où une question
 * s'écrit lettre à lettre, dans ChatGPT puis Perplexity puis Gemini. Ces trois
 * questions étaient écrites en dur — « meilleur restaurant près de moi » pour
 * tout le monde, y compris pour un éditeur de logiciel. La démonstration
 * tombait alors à plat au pire moment : le visiteur attend une analyse de SON
 * site, et l'écran lui parle du commerce de quelqu'un d'autre.
 *
 * Elles sont donc écrites par DeepSeek Flash à partir de la niche détectée et
 * du nom du commerce. C'est un travail d'extraction et de reformulation courte,
 * pas de jugement : le palier rapide suffit, et il est le seul tenable — cet
 * appel est en concurrence directe avec l'analyse elle-même pour l'attention du
 * visiteur, et une question qui arrive après la fin de l'attente n'aura jamais
 * été lue.
 *
 * Rien n'en dépend : si le modèle est indisponible, lent ou hors format, le
 * repli local (`fallbackQuestions`) rend des questions correctes construites sur
 * la niche et la ville. Un écran d'attente n'a pas le droit de faire échouer
 * l'analyse qu'il fait patienter.
 */

/** Trois questions : une par moteur montré sur l'écran d'attente. */
export const QUESTION_COUNT = 3;

/**
 * Ce que le modèle doit rendre.
 *
 * `niche` est demandée en retour même quand on la lui a donnée : appelé sur une
 * simple URL — c'est le cas depuis la page d'accueil, où l'analyse n'a pas
 * encore tourné —, c'est lui qui la déduit, et l'écran d'attente l'affiche.
 */
 ...
 *
 * Elles ne sont pas un pis-aller honteux : ce sont les trois formes que prend
 * réellement la question d'un client — le superlatif, la demande de conseil, et
 * la vérification d'un nom qu'on lui a donné. Le modèle fait mieux parce qu'il
 * connaît le vocabulaire du métier, pas parce que ces trois-là seraient fausses.
 */
...
/**
 * Ramène la réponse du modèle à trois questions utilisables.
 *
 ...
 * avec le repli si le compte n'y est pas : l'écran attend exactement trois
 * questions, une par moteur.
 */
function normalize(raw: string[], input: NicheQuestionsInput): string[] {
  ...
  return unique.slice(0, QUESTION_COUNT);
}

/**
 * Mémoire courte, par domaine.
 *
 * Le même visiteur peut relancer une analyse, et l'écran d'attente est monté à
 * chaque fois. Sans ce cache, chaque montage repayerait le même appel pour la
 * même réponse. Un quart d'heure suffit : au-delà, la niche a pu être affinée
 * par l'analyse elle-même, et il vaut mieux redemander.
 */
const CACHE_TTL_MS = 15 * 60 * 1000;
const cache = new Map<string, { at: number; value: NicheQuestions }>();

/**
 * Trois questions qu'un client taperait pour trouver ce commerce.
 *
  ...
  try {
    const answer = await askJson(schema, {
      ...
      prompt: [
        ...
        "",
        `Écris ${QUESTION_COUNT} questions différentes que de vrais clients poseraient à ChatGPT, Perplexity ou Gemini et dont la réponse devrait citer ce commerce.`,
        `- La première et la deuxième portent sur la niche, sans nommer le commerce : le client ne le connaît pas encore, il cherche un prestataire.`,
        `- La troisième nomme « ${name} » : c'est le client qui vérifie une adresse qu'on lui a conseillée.`,
        input.isPhysical
          ? "- Situe les deux premières sur la ville quand elle est connue."
          : "",
    ...
    // et on le note, sans remonter l'erreur à l'analyse en cours.

...
export const findProspectsAction = authActionClient
  ...
  .action(async ({ ctx }) => {
    const userId = ctx.auth.user.id;
    // La prospection est un appel au modèle, et la page qui l'affiche est
    // réservée au Tout-en-un : sans ce refus, un appel direct à l'action
    // ferait payer une recherche pour un écran qui reste voilé.
